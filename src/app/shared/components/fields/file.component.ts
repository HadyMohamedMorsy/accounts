      ...
      >
        <ng-template pTemplate="content" let-files>
          @if (files.length > 0) {
            <div
              [ngClass]="{
                'flex-wrap': props.accept === 'image/*',
                'flex-column': props.accept !== 'image/*'
              }"
              class="flex gap-3"
            >
              @for (file of files; track $index; let i = $index) {
                @if (props.accept !== 'image/*') {
                  ...
                  >
                    @if (file.name) {
                      <span [pTooltip]="file.name" tooltipPosition="top">
                        {{ file.name | truncateText: 30 }}
                      </span>
                    }
                    @if (file.objectURL) {
                      <span [pTooltip]="file.objectURL" tooltipPosition="top">
                    ...
                    <button
                      pButton
                      class="p-button-rounded bg-blue-500
                            hover:bg-blue-300 w-1.5rem h-1.5rem p-0"
                      type="button"
