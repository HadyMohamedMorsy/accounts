...
export class LangRepeaterFieldService {
  ...
  getlangRepeaterField(
    ...
    modelCallback?: (field: FormlyFieldConfig) => void,
  ): FormlyFieldConfig {
    return {
      key: 'trans',
      type: 'repeat-field',
      props: {
        addBtnText: 'Add Translation',
      ...
      hooks: {
        onInit: (field: FormlyFieldConfig) => {
          ...
          return field.formControl?.valueChanges.pipe(
            ...
            tap((langs) => {
              const firstLangField = field?.parent
                ?.get?.('trans')
                ?.fieldGroup?.[0].get?.('language_code');
              firstLangField?.props && (firstLangField.props.disabled = true);

              const LangFields = field?.parent
                ?.get?.('trans')
                ?.fieldGroup?.slice(1);
              LangFields?.forEach((field) => {
                ...
                fields?.props &&
                  (fields.props.options =
                    this.#staticDataService.languages.slice(1));
              });
